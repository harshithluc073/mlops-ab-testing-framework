...
class ABTestFramework:
    ...
    def explain_models(
        ...
        # Get feature names
        feature_cols = [col for col in self.test_data.columns if col != self.config.experiment.target_column]
        X = self.test_data[feature_cols].values
        ...
        for model_name, model_obj in self.models.items():
            ...
            if method in ['shap', 'both']:
                try:
                    ...
                    shap_result = explain_model_with_shap(
                        model=model_obj,
                        X=X,
                    ...
                    explanations[f'{model_name}_shap'] = shap_result
                    logger.info(f"  SHAP explanation complete")
                    
                    # Show top features
                    top_features = shap_result.get_top_features(5)
            ...
            if method in ['lime', 'both']:
                try:
                    ...
                    lime_result = explain_model_with_lime(
                        model=model_obj,
                        X=X,
                        feature_names=feature_cols,
                        model_name=model_name,
                    ...
                    explanations[f'{model_name}_lime'] = lime_result
                    logger.info(f"  LIME explanation complete")
                    
                except Exception as e:
                    logger.warning(f"  LIME failed: {e}")
        
        # Compare explanations if multiple models
        if len(self.models) >= 2:
            logger.info("\nComparing model explanations...")
            try:
                from mlops_ab_testing.explainability import summarize_comparison
                
                model_names = list(self.models.keys())
                if method in ['shap', 'both']:
                    result_a = explanations.get(f'{model_names[0]}_shap')
                    result_b = explanations.get(f'{model_names[1]}_shap')
                    
                    if result_a and result_b:
                        comparison = summarize_comparison(result_a, result_b)
                        explanations['comparison'] = comparison
                        
                        logger.info(f"  Agreement: {comparison['agreement']['overlap_percentage']:.1f}% overlap in top 10")
                        logger.info(f"  Correlation: {comparison['agreement']['rank_correlation']:.3f}")
